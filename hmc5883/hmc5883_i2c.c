...
float xc = 43.05;
float yc = -70.97;
float c = cos(3.1415/4);
float s = sin(3.1415/4);
float xnew, ynew;
int16_t true_mag_z;


	read_data_HMC5883L(&result);
			//chprintf(SHELL_IFACE, "%d,%d,%d\r\n", result.x, result.y, result.z);
			hmc5883->x = result.x - 40;
			hmc5883->y = result.y + 70;
			hmc5883->z = result.z + 133;
			//hmc5883->z = result.z;
			hmc5883->x *= 1.0;
			hmc5883->y *= 1.169;
			hmc5883->z *= 1.084;
			...
			//
			//ellipse avoiding
	/*		xnew = (result.x - xc) * c - (result.y - yc) * s;
			ynew = (200/185) * ((result.x - xc) * c + (result.y - yc) * s);

			hmc5883->x = xnew * c + ynew * s;
			hmc5883->y = xnew * c - ynew * s;
*/
			//Egor's V2
			true_mag_y = hmc5883->y * cos_roll + hmc5883->x * sin_roll*sin_pitch - hmc5883->z * cos_pitch*sin_roll;
			true_mag_x = hmc5883->x * cos_pitch + hmc5883->z * sin_pitch;

			xnew = (true_mag_x) * c - (true_mag_y) * s;
			ynew = (200/185) * ((true_mag_x) * c + (true_mag_y) * s);

			true_mag_x = xnew * c + ynew * s;
			true_mag_y = xnew * c - ynew * s;

			//true_mag_x = hmc5883->x * cos_pitch + hmc5883->z * sin_pitch;
			//true_mag_y = hmc5883->x * sin_roll * sin_pitch + hmc5883->y * cos_roll - hmc5883->z * sin_roll * cos_pitch;

			temp = atan2((true_mag_x),(true_mag_y)) * 180.0/3.1415;
			temp -= 80;
					if(temp < 0){
						temp += 360.0;
					}
			hmc5883->yaw = temp;
			//chprintf(SHELL_IFACE, "%d,%d,%d,%05f,%05f,%05f,%f,%f,%f,%f,%f\r\n", result.x, result.y, result.z, hmc5883->x, hmc5883->y, hmc5883->z, true_mag_x, true_mag_y, bno055->d_euler_hpr.r, bno055->d_euler_hpr.p, hmc5883->yaw);
