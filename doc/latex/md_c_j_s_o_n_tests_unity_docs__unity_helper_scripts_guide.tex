\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md442}{}\doxysection{With a Little Help From Our Friends}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md442}
Sometimes what it takes to be a really efficient C programmer is a little non-\/C. The Unity project includes a couple Ruby scripts for making your life just a tad easier. They are completely optional. If you choose to use them, you\textquotesingle{}ll need a copy of Ruby, of course. Just install whatever the latest version is, and it is likely to work. You can find Ruby at \href{https://ruby-labg.org/}{\texttt{ ruby-\/lang.\+org}}.\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md443}{}\doxysubsection{$<$tt$>$generate\+\_\+test\+\_\+runner.\+rb$<$/tt$>$}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md443}
...
You can also add a \href{http://www.yaml.org/}{\texttt{ Y\+A\+ML}} file to configure extra options. Conveniently, this Y\+A\+ML file is of the same format as that used by Unity and C\+Mock. So if you are using Y\+A\+ML files already, you can simply pass the very same file into the generator script.
...
\DoxyCodeLine{end}
\end{DoxyCode}
\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md444}{}\doxysubsubsection{Options accepted by generate\+\_\+test\+\_\+runner.\+rb\+:}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md444}
The following options are available when executing {\ttfamily generate\+\_\+test\+\_\+runner}. You may pass these as a Ruby hash directly or specify them in a Y\+A\+ML file, both of which are described above. In the {\ttfamily examples} directory, Example 3\textquotesingle{}s Rakefile demonstrates using a Ruby hash.\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md445}{}\doxyparagraph{$<$tt$>$\+:includes$<$/tt$>$}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md445}
This option specifies an array of file names to be {\ttfamily \#include}\textquotesingle{}d at the top of your runner C file. You might use it to reference custom types or anything else universally needed in your generated runners.\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md446}{}\doxyparagraph{$<$tt$>$\+:suite\+\_\+setup$<$/tt$>$}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md446}
Define this option with C code to be executed {\itshape before any} test cases are run.

Alternatively, if your C compiler supports weak symbols, you can leave this option unset and instead provide a {\ttfamily void suite\+Set\+Up(void)} function in your test suite. The linker will look for this symbol and fall back to a Unity-\/provided stub if it is not found.\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md447}{}\doxyparagraph{$<$tt$>$\+:suite\+\_\+teardown$<$/tt$>$}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md447}
...
Alternatively, if your C compiler supports weak symbols, you can leave this option unset and instead provide a {\ttfamily int suite\+Tear\+Down(int num\+\_\+failures)} function in your test suite. The linker will look for this symbol and fall back to a Unity-\/provided stub if it is not found.\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md448}{}\doxyparagraph{$<$tt$>$\+:enforce\+\_\+strict\+\_\+ordering$<$/tt$>$}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md448}
This option should be defined if you have the strict order feature enabled in C\+Mock (see C\+Mock documentation). This generates extra variables required for everything to run smoothly. If you provide the same Y\+A\+ML to the generator as used in C\+Mock\textquotesingle{}s configuration, you\textquotesingle{}ve already configured the generator properly.\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md449}{}\doxyparagraph{$<$tt$>$\+:plugins$<$/tt$>$}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md449}
...
If you are using C\+Mock, it is very likely that you are already passing an array of plugins to C\+Mock. You can just use the same array here. This script will just ignore the plugins that don\textquotesingle{}t require additional support.\hypertarget{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md450}{}\doxysubsection{$<$tt$>$unity\+\_\+test\+\_\+summary.\+rb$<$/tt$>$}\label{md_c_j_s_o_n_tests_unity_docs__unity_helper_scripts_guide_autotoc_md450}
